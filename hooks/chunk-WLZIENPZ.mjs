...
function matchPages(prompt, pagesDir, max = 3) {
...
function scopePaths(key) {
  ...
  const projectDir = join3(home, "projects", key);
  const globalDir = join3(home, "global");
  return {
    projectDir,
    globalDir,
    inboxFile: join3(projectDir, "inbox.md"),
    logFile: join3(projectDir, "log.md"),
    pagesDir: join3(projectDir, "pages")
  };
}
function storeExists() {
  return pathExists(join3(mehmoryHome(), "global", "identity.md"));
}
function storeIsUnpopulated(key) {
  const paths = scopePaths(key);
  if (readIfPresent(join3(paths.projectDir, "project.md")) !== "") return false;
  for (const dir of [paths.pagesDir, join3(paths.globalDir, "pages")]) {
...
function buildScopeInjection(key, config = loadConfig()) {
  return failOpen(
    () => {
      const paths = scopePaths(key);
      const projectIndex = join3(paths.projectDir, "index.md");
      const frame = buildInjection([
        { label: "identity", content: readIfPresent(join3(paths.globalDir, "identity.md")) },
        { label: "project", content: readIfPresent(join3(paths.projectDir, "project.md")) },
        {
          label: "index",
          content: readIfPresent(
            pathExists(projectIndex) ? projectIndex : join3(paths.globalDir, "index.md")
          )
        }
...
function distillDelta(sessionId, transcriptPath, config = loadConfig()) {
...
function captureDelta(sessionId, transcriptPath, key, config = loadConfig()) {
...
function rememberEntry(text, sessionId, config = loadConfig()) {
