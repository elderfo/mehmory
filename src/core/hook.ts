 ...
 * issue #25) and recorded on the stats line, never read ambiently by the body itself.
 * A disabled harness skips `body` entirely — no stdin is read, so there is no capture,
 * no injection and no pointer, the same silence `/mehmory:pause` promises for a single
 * session, but persistent and scoped to one harness via config instead.
 *
